...
public final class GeoVeilEntry {
    ...
    private static final Object DECOR_TAG = "geoveil-manager-attached-v1";
    ...
    private static boolean shouldAttach(Activity activity) {
        if (activity == null || !SHELL_PACKAGE.equals(activity.getPackageName())) {
            return false;
        }
    ...
    private static void attach(Activity activity) {
        if (!shouldAttach(activity) || activity.isFinishing() || activity.isDestroyed()) {
            return;
        }
        View decor = activity.getWindow().getDecorView();
        if (DECOR_TAG.equals(decor.getTag())) {
            return;
        }
        ...
        activity.setContentView(new ManagerScreen(activity));
    }

    /**
     * Covers the race where the Shell trampoline finished onCreate before the lifecycle callback
     * was registered. This is best-effort only; failure leaves the stock Shell activity untouched.
     */
    private static void attachExistingActivityBestEffort() {
        try {
            ...
            // Hidden implementation details differ across Android builds. Normal lifecycle
            // callbacks remain the supported path and this fallback must never crash Shell.
        }
    }

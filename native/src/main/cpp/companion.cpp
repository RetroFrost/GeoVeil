...
constexpr char kModuleDirectory[] = "/data/adb/modules/geoveil";
...
bool ensure_directory(const char* path, mode_t mode) {
    if (mkdir(path, mode) == 0 || errno == EEXIST) {
        return chmod(path, mode) == 0 || errno == EPERM;
    }
    return false;
}
...
bool send_response_with_fds(int socket_fd, const Response& response, const int* fds, size_t fd_count) {
    iovec io{};
...
public:
    ...
    bool ready() const {
        return state_fd_ >= 0 && shared_ != nullptr;
    }
    ...
    Response probe() {
        ...
        Response response;
        if (!read_snapshot(shared_, &current)) {
            response.status = static_cast<int32_t>(Status::kIoError);
            return response;
        }
        response.status = static_cast<int32_t>(Status::kOk);
        response.flags = current.flags;
        response.accepted_generation = current.generation;
        return response;
    }
    ...
    Response publish(uint64_t generation, const PublishPayload& payload) {
        ...
        if (generation <= current.generation) {
            response.status = static_cast<int32_t>(Status::kStaleGeneration);
            response.flags = current.flags;
            response.accepted_generation = current.generation;
            return response;
        }
        ...
        if (!validate_snapshot(next)) {
            response.status = static_cast<int32_t>(Status::kInvalidState);
            response.flags = current.flags;
            response.accepted_generation = current.generation;
            return response;
        }
        ...
        response.accepted_generation = next.generation;
        return response;
    }
    ...
    Response move(uint64_t generation, const MovePayload& payload) {
        ...
        if ((current.flags & (kFlagEmergencyDisable | kFlagEnabled | kFlagHasCoordinates))
                != (kFlagEnabled | kFlagHasCoordinates)) {
            response.status = (current.flags & kFlagEmergencyDisable) != 0
                    ? static_cast<int32_t>(Status::kEmergencyDisabled)
                    : static_cast<int32_t>(Status::kNotReady);
            response.flags = current.flags;
            response.accepted_generation = current.generation;
            return response;
        }
        ...
        if (magnitude > 0.0001 && delta_seconds > 0.0) {
            ...
            const double latitude_radians = current.latitude * M_PI / 180.0;
            ...
            next.latitude = std::clamp(current.latitude + latitude_delta * 180.0 / M_PI,
                    -89.999999, 89.999999);
            next.longitude = current.longitude + longitude_delta * 180.0 / M_PI;
            while (next.longitude > 180.0) next.longitude -= 360.0;
            while (next.longitude < -180.0) next.longitude += 360.0;
            double bearing = std::atan2(unit_x, -unit_y) * 180.0 / M_PI;
    ...
    Response engine_begin(uint64_t generation, int32_t pid) {
        ...
        if (path_exists(kEmergencyMarker) || has_unfinished_generation()) {
            ...
            current.flags &= ~(kFlagEnabled | kFlagEngineReady);
        ...
        if (!write_marker(kInstallingMarker, generation, pid)) {
            response.status = static_cast<int32_t>(Status::kIoError);
            return response;
        }
    ...
    Response engine_healthy(uint64_t generation, int32_t pid) {
        ...
        if (!write_marker(kHealthyMarker, generation, pid)) {
            response.status = static_cast<int32_t>(Status::kIoError);
            return response;
        }
    ...
    Response clear_emergency() {
        ...
        if ((current.flags & kFlagEnabled) != 0 || path_exists(kInstallingMarker)) {
            response.status = static_cast<int32_t>(Status::kNotReady);
            response.flags = current.flags;
            response.accepted_generation = current.generation;
            return response;
        }
...
Response handle_message(CompanionState& state, const ClientHello& hello,
    ...
    if (header.magic != kIpcMagic || header.version != kIpcVersion) {
        Response response;
        response.status = static_cast<int32_t>(Status::kBadVersion);
        return response;
    }
    ...
    switch (static_cast<Operation>(header.operation)) {
        ...
        case Operation::kPublish:
            if (header.payload_size != sizeof(PublishPayload) || payload == nullptr) break;
            return state.publish(header.generation, *static_cast<const PublishPayload*>(payload));
        case Operation::kMove:
            if (header.payload_size != sizeof(MovePayload) || payload == nullptr) break;
    ...
    Response response;
    response.status = static_cast<int32_t>(Status::kBadMessage);
    return response;
}
...
void companion_handler(int client) {
    ...
    if (!send_response_with_fds(client, initial, fd_count == 0 ? nullptr : fds, fd_count)) {
    ...
    for (;;) {
        ...
        if (header.payload_size > sizeof(payload)) {
            ...
            write_full(client, &bad, sizeof(bad));
            break;
        }
        if (header.payload_size > 0 && !read_full(client, payload, header.payload_size)) {
            break;
        }
        Response response = handle_message(state, hello, header,
                header.payload_size > 0 ? payload : nullptr);
        if (!write_full(client, &response, sizeof(response))) {
            break;
        }
    }
